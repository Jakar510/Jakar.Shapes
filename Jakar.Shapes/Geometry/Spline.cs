...
// arguments by reflection, which cannot pass a ref struct. Spline also has to own an array, since a struct
...
public readonly struct Spline( params ReadOnlyPoint[]? points ) : ISpline<Spline>
{
    ...
    /// <summary>
    /// Copies <paramref name="points"/> into a new spline, so callers can pass a stackalloc buffer.
    /// <para>
    /// The copy is unavoidable: a spline owns its points for its whole lifetime, and a struct cannot hold a
    ...
    /// </para>
    /// </summary>
    public Spline( params ReadOnlySpan<ReadOnlyPoint> points ) : this(points.IsEmpty
                                                                       ? __empty
                                                                       : [.. points]) { }
    ...
    [Pure] public static Spline Create( params ReadOnlySpan<ReadOnlyPoint> points ) => new(points);
